...
ProducerStream.prototype._write_message = function(message, encoding, cb) {
  ...
  const topic = message.topic ? message.topic : this.topicName;

  try {
    this.producer.produce(topic, message.partition, message.value, message.key, message.timestamp, message.opaque, message.headers);
    setImmediate(cb);
  } catch (e) {
    if (ErrorCode.ERR__QUEUE_FULL === e.code) {
      // Poll for good measure
